...
class ACNet(object):
    ...
    def update_global(self, feed_dict, fetches=[]):  # run by a local
        return SESS.run([self.update_a_op, self.update_c_op] + fetches, feed_dict)[2:]  # local grads applies to global net
...
class Worker(object):
    def __init__(self, name, globalAC):
        self.env = gym.make(config.config.GAME_NAME)
    ...
    def work(self):
        ...
        while not COORD.should_stop() and GLOBAL_EP < MAX_GLOBAL_EP:
            ...
            if self.name == 'W_0' and (GLOBAL_EP+1) % 10000 == 0 and RUN_MODE == 'training':
                saver.save(SESS, ".tf-models/a3c", global_step=GLOBAL_EP)
            ...
            while True:
                ...
                if RUN_MODE == "training" and (total_step % UPDATE_GLOBAL_ITER == 0 or done):  # update global and assign to local net
                    ...
                    a_loss = self.AC.update_global(feed_dict, [self.AC.a_loss])
                    # print(a_loss)
